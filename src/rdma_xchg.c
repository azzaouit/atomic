...
int __qp_connect(struct rdma_ctx *r, struct node_config *c) {
    ...
    struct remote_attr *ra = r->ra + c->id;
    ...
    // set QP to RTR state
    ret = ibv_modify_qp(r->qp[c->id], &rtr_attr,
    ...
    // set QP to RTS state
    ret = ibv_modify_qp(r->qp[c->id], &rts_attr,
